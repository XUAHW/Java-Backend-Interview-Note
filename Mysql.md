...
很显然这条查询语句是**覆盖索引**，直接在二级索引就能查找到结果（因为二级索引的 B+ 树的叶子节点的数据存储的是主键值，查询只要求返回`id`字段，而二级索引的叶子节点**已经包含了id值**，只需扫描`idx_name`索引的叶子节点，找到所有`name LIKE 'i%'`的记录，直接从中提取`id`值返回，**全程无需回表**（不需要根据主键id去主键索引查找数据行）），就没必要在主键索引查找了，因为查询主键索引的 B+ 树的成本会比查询二级索引的 B+ 的成本大，优化器基于查询成本的考虑，会选择查询代价小的普通索引。
